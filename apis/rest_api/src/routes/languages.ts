...
import { findAllLanguagesById, updateLanguage, removeLanguage, addLanguageInfo } from '../mongoFunctions';
...
router.put('/languages/:id', [], async (req: Request, res: Response) => {
    const response = await updateLanguage(parseInt(req.params.id), req.body.languages);
    return res.status(200).send(response);
});
